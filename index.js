...
function encodeConsonantWord(word) {
    ...
    */
    if (!(word[0].match(vowels))) {
        console.log(word);
        const findVowel = word.indexOf(word.match(vowels)[0]);
        wordConsonantFirstPigLatined = word.substr(findVowel) + "-" + word.substr(0, findVowel) + "ay";
        console.log(wordConsonantFirstPigLatined)
    }
    return wordConsonantFirstPigLatined;
}
...
document.getElementById("button").onclick = function() {
    ...
    let pigResult = document.getElementById("textInput").value;
    pigResult = document.createTextNode(encodeText(pigResult));
    span.appendChild(pigResult);
    document.getElementById("toPig").appendChild(span);
}
...
function decodeConsonantWord(word) {
    word = word.slice(0, -2);
    let lastConsonant = word.split("-")[1];
    word = lastConsonant + word;
    word = word.split("-")[0];
    console.log(word);
    return word;
}
...
document.getElementById("button2").onclick = function() {
    ...
    let engResult = document.getElementById("textInput2").value;
    engResult = document.createTextNode(decodeText(engResult));
    span2.appendChild(engResult);
    document.getElementById("toEng").appendChild(span2);
}
